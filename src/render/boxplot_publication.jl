...
function hide_every_extra!(ax::Axis)
    hide_grid!(ax)
    hide_ticks_and_spine!(ax)
end

function hide_everything_except_xaxis!(ax::Axis; spine_as_well=true)
    hide_y_ticks!(ax)
    hide_ylabels!(ax)
    hide_grid!(ax)
    if spine_as_well
...
function hide_everything!(ax::Axis; spine_as_well=true)
    hide_x_ticks!(ax)
    hide_xlabels!(ax)
    hide_y_ticks!(ax)
    hide_ylabels!(ax)
    ...
    if t == 1 # singleton
        ...
        if length(distances) < max_display_num
            for (index, d) in enumerate(distances)
                logo_paths[index] = joinpath(parent_save_path, "$(get_d_str(d))_.png")
...
function setup_plots!(ax_lefts, ax_rights, grid, logo_paths, contribs; 
    ...
    for r = 1:how_many_rows
        ...
        CairoMakie.scatter!(ax_rights[r], 
            box_data, 0.035 .* randn(length(box_data)) .+ 0.35 , color = :maroon, markersize = boxplot_marker_size)
